...
#8.3 Explorando as variáveis numéricas 
...
#porém na explicação os dados somente vão até 5
unique(dados_num$SIZE)
#vamos corrigir antes de prosseguir
range(dados_num$SIZE)
unique(dados_num$SIZE)
dim(dados_num)
...
#8.4 Explorando as variáveis categóricas
...
barplot(table(dados_fator$FUEL), main = "Gráfico de Barras", xlab = "Categorias", ylab = "Contagem", col = "skyblue", ylim = c(0, max(table(dados_fator$FUEL)) + 1))
...
barplot(table(dados_fator$STATUS), main = "Gráfico de Barras", xlab = "Categorias", ylab = "Contagem", col = "skyblue", ylim = c(0, max(table(dados_fator$STATUS)) + 1))
...
#algoritmos: REGRESSÃO LOGÍSTICA, SVM, DECISION FOREST, 
# NEURAL NETWORK, BOOSTED DECISION TREE,
...
#9.5 Modelo de Boosted Tree Decision
...
# 10 - Otimização de Modelo de Machine Learning
...
#modelo final com acurácia de 96,3%
